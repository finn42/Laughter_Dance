...
def Inspiration_Extract(sig,filtered=False,vel_c = 0.5,x_hz = 0.2):
    ...
    cutoff = x_hz
    nyq = 0.5 * sf # 0.5 as in 2 second cycles. That filters out laughter. hm 
    order = 2 
    normal_cutoff = cutoff / nyq
...
def breath_cycles(Breaths2,R):
    df_sig = pd.DataFrame(R.values,index=R.index)
    df_sig = df_sig.rename(columns={0:'Breathing'})
    cutoff = 8
    sf = round(1/np.nanmean(np.diff(R.index)))
    b, a = butter(2, cutoff/(0.5*sf), btype='low', analog=False)
    K = df_sig['Breathing'].diff().diff()
    L =  df_sig['Breathing'].diff()
    G = pd.DataFrame(index = K[K.notna()].index)
    G['Resp'] = df_sig.loc[K.notna(),'Breathing']
    G['diffResp'] = filtfilt(b,a,L[K.notna()])
    G['diffdiffResp'] = filtfilt(b,a,K[K.notna()])
    ...
    if len(Breaths2)>1:        
        ...
        Br['Ipre_Ratio'] = Br['Insp_T']/Br['Exp_T'].shift(1) 
        ...
        Br['IE_VRlog'] = np.log(Br['Insp_V']/Br['Exp_V'].abs()) #just to reduce the range
        Br['RelID'] = Br['IDepth']/Br['IDepth'].rolling(5,center=True).median()
        Br['RelED'] = Br['EDepth']/Br['EDepth'].rolling(5,center=True).median()
        
        # Extra segment features
        Br.loc[:,['Vel_In_Max','Vel_Ex_Min','Acc_In_Max','Acc_Ex_Min',
                  'Vel_In_rMean','Vel_Ex_rMean','Acc_In_rMean','Acc_Ex_rMean',
                  'Vel_In_CrN','Vel_Ex_CrN','Acc_In_CrN','Acc_Ex_CrN']] = 0.0
        for i in range(len(Br)-1):
            seg_in = G.loc[Br['In'][i]:Br['Ex'][i],:]
            seg_ex = G.loc[Br['Ex'][i]:Br['In'][i+1],:]
            Br.loc[i,'Vel_In_Max'] = seg_in['diffResp'].max()
            Br.loc[i,'Acc_In_Max'] = seg_in['diffdiffResp'].max()
            Br.loc[i,'Vel_Ex_Min'] = seg_ex['diffResp'].min()
            Br.loc[i,'Acc_Ex_Min'] = seg_ex['diffdiffResp'].min()
            Br.loc[i,'Vel_In_rMean'] = seg_in['diffResp'].abs().mean()
            Br.loc[i,'Vel_Ex_rMean'] = seg_ex['diffResp'].abs().mean()
            Br.loc[i,'Acc_In_rMean'] = seg_in['diffdiffResp'].abs().mean()
            Br.loc[i,'Acc_Ex_rMean'] = seg_ex['diffdiffResp'].abs().mean()
            A = np.sign(seg_in['diffResp']).diff().abs()
            Br.loc[i,'Vel_In_CrN'] = A[A>1].sum()/2
            A = np.sign(seg_ex['diffResp']).diff().abs()
            Br.loc[i,'Vel_Ex_CrN'] = A[A>1].sum()/2 
            A = np.sign(seg_in['diffdiffResp']).diff().abs()
            Br.loc[i,'Acc_In_CrN'] = A[A>1].sum()/2
            A = np.sign(seg_ex['diffdiffResp']).diff().abs()
            Br.loc[i,'Acc_Ex_CrN'] = A[A>1].sum()/2
        # cleaning up spurious calculation errors
        Br.replace([np.inf, -np.inf], np.nan, inplace=True)
        
        Br['dPT'] = np.log(Br['Period_T']/Br['Period_T'].shift(1)).abs()
        Br['dIT'] = np.log(Br['Insp_T']/Br['Insp_T'].shift(1)).abs()
        Br['dID'] = np.log(Br['IDepth']/Br['IDepth'].shift(1)).abs()
...
def breathcats(Insp_seq):
    Insp_seq['Categories']='Dis'
    Insp_seq['CatN']=1

    V = Insp_seq.query('Insp_T<1.2 and Period_T<4 and IE_Ratio>0.5')
    Insp_seq.loc[V.index,'Categories']='Work'
    Insp_seq.loc[V.index,'CatN']=0
    # V = Insp_seq.query('Insp_T>0.5 and Insp_T<1 and Period_T>1.75 and IE_Ratio>0.3 and dPT<0.5 and dID<0.3')
    # Insp_seq.loc[V.index,'Categories']='Steady'
    # Insp_seq.loc[V.index,'CatN']=2

    V = Insp_seq.query('Insp_T<0.8 and Exp_T>1.8 and IE_VRlog>1.8')
    Insp_seq.loc[V.index,'Categories']='Speech'
    Insp_seq.loc[V.index,'CatN']=-1
    V = Insp_seq.query('Acc_Ex_CrN>20')
    Insp_seq.loc[V.index,'Categories']='Speech'
    Insp_seq.loc[V.index,'CatN']=-1
    # abdnominal laughter
    V = Insp_seq.query('Vel_Ex_rMean>5 and Exp_T>2')
    Insp_seq.loc[V.index,'Categories']='Laugh'
    Insp_seq.loc[V.index,'CatN']=-2
    # post laugh gasp
    V = Insp_seq.query('Ipre_Ratio<0.3 and Vel_In_Max>35')
    Insp_seq.loc[V.index,'Categories']='Gasp'
    Insp_seq.loc[V.index,'CatN']=-3
    Insp_seq.loc[V.index-1,'Categories']='Laugh'
    Insp_seq.loc[V.index-1,'CatN']=-2


    return Insp_seq.loc[:,['Categories','CatN']]